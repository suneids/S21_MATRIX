...
int s21_create_matrix(int rows, int columns, matrix_t *result) {
  ...
  if (result_status == OK) {
    ...
    temp.matrix = (double **)calloc(rows, sizeof(double *));
    if (temp.matrix == NULL) result_status = INCORRECT_MATRIX_ERROR;
    for (int i = 0; i < rows && !result_status; i++) {
      temp.matrix[i] = (double *)calloc(columns, sizeof(double));
      if (temp.matrix[i] == NULL) result_status = INCORRECT_MATRIX_ERROR;
    }
...
int s21_mult_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
  int result_status = OK;
  if (s21_matrix_is_not_null(A) && s21_matrix_is_not_null(B)) {
    if (A->columns != B->rows) result_status = CALCULATION_ERROR;
...
int s21_transpose(matrix_t *A, matrix_t *result) {
  int result_status = OK;
  matrix_t temp;
  if (!s21_matrix_is_not_null(A)) result_status = INCORRECT_MATRIX_ERROR;
...
int s21_inverse_matrix(matrix_t *A, matrix_t *result) {
  ...
  if (!result_status) {
    ...
    if (!result_status && determinant == 0) result_status = CALCULATION_ERROR;
    if (!result_status) {
